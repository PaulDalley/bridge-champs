/**
 * Funnel link-equity into the Transfers over 1C article: insert a link as the
 * FIRST "Read next" item in the related transfer/convention articles (Firestore).
 * Nav-only edit — never touches bridge prose. Idempotent. Backs up originals.
 *
 * Dry-run:  node scripts/_crosslink-transfers-1c.js
 * Apply:    node scripts/_crosslink-transfers-1c.js --apply
 */
...
const INSERT = '<a href="/learn/bidding/transfers-over-1c">Transfers over 1C: How They Work</a> &middot; ';
const SIBLINGS = ["transfers-must-have-tool-nt", "system-over-1nt-conventions", "fourth-suit-forcing-checkback-core"];
...
(async () => {
  ...
  for (const slug of SIBLINGS) {
    ...
    if (text.includes("/learn/bidding/transfers-over-1c")) { console.log("SKIP (already links it): " + slug); continue; }
    ...
    console.log("\n### " + slug + "  (body " + bodyId + ")");
    console.log("  " + newText.slice(i, i + 220).replace(/\n/g, " "));
    if (APPLY) {
  ...
  if (APPLY && Object.keys(backup).length) {
    fs.writeFileSync(path.join(__dirname, "_transfers1c-crosslink-backup.json"), JSON.stringify(backup, null, 2));
    console.log("\nBackup -> scripts/_transfers1c-crosslink-backup.json");
  }
