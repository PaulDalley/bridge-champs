/**
 * Funnel link-equity into the checkback article: insert a link as the FIRST
 * "Read next" item in the related convention articles (Firestore).
 * Nav-only edit — never touches bridge prose. Idempotent. Backs up originals.
 *
 * Dry-run:  node scripts/_crosslink-checkback.js
 * Apply:    node scripts/_crosslink-checkback.js --apply
 */
...
const INSERT = '<a href="/learn/bidding/two-way-checkback">Two-way checkback: all you need to know</a> &middot; ';
const SIBLINGS = ["fourth-suit-forcing-checkback-core", "system-over-1nt-conventions"];
...
(async () => {
  ...
  for (const slug of SIBLINGS) {
    ...
    if (text.includes("/learn/bidding/two-way-checkback")) { console.log("SKIP (already links it): " + slug); continue; }
  ...
  if (APPLY && Object.keys(backup).length) {
    fs.writeFileSync(path.join(__dirname, "_checkback-crosslink-backup.json"), JSON.stringify(backup, null, 2));
    console.log("\nBackup -> scripts/_checkback-crosslink-backup.json");
  }
