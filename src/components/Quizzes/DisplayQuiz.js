...
import {startSubmitUserQuizScore, getUserQuizScore, setCurrentQuizScore} from '../../store/actions/usersActions';
...
export class DisplayQuiz extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.quizScores) {
            ...
            if (existingScore && !this.state.redoCompletedQuiz) {
                this.setState({score: existingScore, registeredScore: existingScore, answer: true, loading: false, submitted: true});
            }
        }
    ...
    componentDidUpdate() {
        ...
        else if (this.props.uid && this.props.quizScoresFetched && !this.props.currentQuizScore) {
        ...
        if (this.props.quizScores) {
            ...
            // console.log(this.props.match.params.id);
            let existingScore = this.props.quizScores[this.props.match.params.id];
            if (!this.state.redoCompletedQuiz && (existingScore || existingScore === 0)) {
    ...
    scoreUserAnswer = ({move, suit}) => {
        ...
        for (let i = 0; i < numAnswers; i++) {
            ...
            if (answers[i].move === move.toString()) {
                // console.log("IN HERE");
                if (!answers[i].suit || answers[i].suit === "None" || (answers[i].suit === suit)) {
    ...
    retryQuiz = () => {
        // console.log(" IN HERE");
        this.setState({score: undefined, answer: false, loading: false, submitted: false, redoCompletedQuiz: true });
    ...
    submitAnswer = (e) => {
        ...
        if (!this.state.redoCompletedQuiz)
            this.props.startSubmitUserQuizScore(this.props.uid, score, this.props.match.params.id);
    }
    ...
    generateAnswerOutputString = () => {
        ...
        if (this.state.registeredScore !== undefined && this.state.score !== this.state.registeredScore) {
            originalScoreString = " Your original score was " + this.state.registeredScore + ".";
        }
        if (this.state.score === "10") {
            return "Congratulations, you have scored 10/10 Champion points for this quiz." + originalScoreString;
        }
        else {
            return "You have scored " + this.state.score + " Champion points." + originalScoreString;
        }
    }
    ...
    render() {
        ...
        if (boardData && quizMetadata) {
            if (quizMetadata.quizType === "Bidding" && !this.state.lastBid) { // "Declarer"
        ...
        if (!this.props.dailyFree &&
            (!this.props.quizScoresFetched || this.state.loading || !this.state.quizQuestion)) {
            return (
        ...
        return (
            ...
            <div className="DisplayArticle-container DisplayQuiz-container">
                ...
                <div>
                    ...
                    <div className="DisplayArticle-category">{quizMetadata.quizType}</div>
                    <div
                        className={`DisplayArticle-difficulty ArticlesListItem-difficulty-${quizMetadata.difficulty}`}>{getDifficultyStr(quizMetadata.difficulty)}</div>
                    {this.state.submitted &&
                    <div className="DisplayArticle-category DisplayQuiz-completed">Completed</div>}
                    ...
                    <div className="DisplayArticle-createdAt">{makeDateString(quizMetadata.date)}</div>
                </div>}
                <br/>
                <br/>
                <br/>
                <div className="DisplayQuiz-content_container">
                    ...
                    <div className="DisplayQuiz-biddingInputArea"
                         style={inputStyles}>
                        ...
                        <div className="DisplayQuiz-biddingGrid">
                            ...
                            <Button className="CreateArticle-submit DisplayQuiz-submit"
                                    onClick={(e) => this.submitAnswer(e)}
                                    waves='light'>Submit Answer
                ...
                <div className="DisplayQuiz-hand_container"
                     style={{position: 'relative', top: '-11rem'}}
                >
                    <div className="DisplayQuiz-hand">
                        <PlayHand hand={this.state.hand}
                                  passClickedCardUp={this.playHandCardClicked}
                        />
                    </div>
                    { this.state.answer &&

                    <Button className="CreateArticle-submit DisplayQuiz-submit-openingLead"
                            onClick={(e) => this.submitAnswer(e)}
                            waves='light'>Confirm Answer: {this.state.answer.move} {this.state.answer.suit}
                        <Icon left>done_all</Icon>
                    </Button>
                    }
                </div>
                ...
                <div style={{position: 'relative', display: 'flex', justifyContent: 'center', alignItems:'center', flexDirection: 'column'}}>
                    ...
                    <div onClick={() => this.retryQuiz()}
                         style={{position: 'relative', top: '-1rem', cursor: "pointer", width: 'auto',marginBottom: '2rem'}}>
                        <Button floating large style={{backgroundColor: 'forestgreen'}} waves='light' icon='undo' />
                        <br/>
                        <span style={{
                            position: 'relative',
                ...
                {this.state.submitted && <div styles={answerStyles}>
...
export default connect(
    ...
    {getQuiz, getQuizMetadata, getUserQuizScore, startSubmitUserQuizScore, setCurrentQuizScore})
(DisplayQuiz);