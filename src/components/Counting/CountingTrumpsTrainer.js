 ...
 */ 
 ...
 if (out.shownHandsOverride) out.shownHandsOverride = remapTopLevelSeatKeys(out.shownHandsOverride); 
...
/** Force which category tab is highlighted, regardless of route/categoryKey (e.g. "beginner"). */ 
activeCategoryKeyOverride = null, 
}) { 
...
const activeCategoryTabKey = useMemo(() => {
if (activeCategoryKeyOverride) return activeCategoryKeyOverride;
const pathname = String(location?.pathname || "");
if (pathname.startsWith("/beginner/practice")) return "beginner";
return categoryKey;
}, [activeCategoryKeyOverride, location?.pathname, categoryKey]);
 ...
 */ 
 ...
 /** Result note carried onto the NEXT screen after an auto-continue: { type: "ok"|"error", text }. */
 ...
 // Optional: reveal extra full hands during an INFO prompt. 
 if (promptStep === "INFO") { 
 ...
 */ 
 ...
 const keepExistingTrickCards = !!puzzle.rounds?.[r]?.keepExistingTrickCards; 
 setQueuedTimeout(() => { 
 if (!keepExistingTrickCards) { 
 setTrickCards({ LHO: null, DUMMY: null, RHO: null, DECLARER: null }); 
 } 
 setRoundIdx(r); 
 setPlayIdx(-1); 
 ...
 const hand = activeCustomPrompt?.shownHandsOverride?.[seat] || puzzle.shownHands?.[seat]; 
 if (!hand) return []; 
 ...
 if (categoryKey === "bidding" && (puzzle?.promptOptions?.hideBiddingHand || activeCustomPrompt?.hideBiddingHand)) { 
 return null; 
 } 
 ...
 const full = activeCustomPrompt?.shownHandsOverride?.[seat] 
 ? buildFullHandCards(activeCustomPrompt.shownHandsOverride[seat]) 
 : initialFullHands[seat] || []; 
...
const revealAfterTwoWrong = (kind, promptId) => {
 ...
 setWrongAttempts((prev) => ({ ...prev, [key]: next })); 
  // Global policy: reveal on first wrong attempt (no retry loop).
  return next >= 1;
 }; 
 ...
 const wrongText =
 isCustomGuess && typeof activeCustomPrompt?.wrongText === "string" && activeCustomPrompt.wrongText.trim()
 ? activeCustomPrompt.wrongText.trim()
 : "";
 const encourageAndContinue = () => {
 if (promptId) {
 askedRef.current = {
 ...(askedRef.current || {}),
 ...
 setFeedback(null);
 setWaitingForContinue(false);
 setCarryNote({ type: "ok", text: wrongText });
 setDistributionSuit(suit);
 ...
 : ""; 
 const autoContinueDist = 
 isCustomGuess && activeCustomPrompt?.autoContinueOnCorrect !== undefined 
 ? !!activeCustomPrompt.autoContinueOnCorrect 
 : !sTxt; 
 if (autoContinueDist) {
 if (wrongText) {
 encourageAndContinue();
 ...
 setCarryNote({
 type: "error",
 text: `Not quite — the answer was ${expectedDist.LHO}${expectedDist.DUMMY}${expectedDist.RHO}${expectedDist.DECLARER}.`,
 });
 setDistributionSuit(suit);
 continueFromRound(completedRoundIdx);
 return;
 ...
 type: "ok",
 text:
 isCustomGuess && typeof activeCustomPrompt?.successText === "string" && activeCustomPrompt.successText.trim()
 ? activeCustomPrompt.successText.trim()
 : "Well done!",
 });
 if (promptId) { 
 ...
 const hasSuccessText = typeof successTextRaw === "string" && successTextRaw.trim() !== ""; 
 const autoContinueOnCorrect = 
 activeCustomPrompt?.autoContinueOnCorrect !== undefined 
 ? !!activeCustomPrompt.autoContinueOnCorrect 
 : !hasSuccessText; 
 const advanceAndMarkAsked = () => { 
 askedRef.current = { 
 ...(askedRef.current || {}), 
 customAsked: { ...((askedRef.current && askedRef.current.customAsked) || {}), [promptId]: true }, 
 }; 
 ...
 setWaitingForContinue(false); 
 setSeatSuitCountInput(""); 
 setCarryNote({ type: "ok", text: hasSuccessText ? successTextRaw.trim() : "Well done!" }); 
 advanceAndMarkAsked(); 
 return; 
 } 
 setFeedback({ type: "ok", text: hasSuccessText ? successTextRaw : getShortSuccess() }); 
 pendingAdvanceRef.current = advanceAndMarkAsked; 
 setWaitingForContinue(true); 
 return; 
 } 
 ...
 setWaitingForContinue(false); 
 setSeatSuitCountInput(""); 
 setCarryNote({ type: "error", text: `Not quite — the answer was ${expected}.` }); 
 advanceAndMarkAsked(); 
 return; 
 } 
 ...
 const successTextRaw = activeCustomPrompt?.successText; 
 const hasSuccessText = typeof successTextRaw === "string" && successTextRaw.trim() !== ""; 
 // Default: a correct answer flows straight to the next screen (with a carried "Well done"), 
 // unless the author set an explicit flag or provided a standalone success message. 
 const autoContinueOnCorrect = 
 activeCustomPrompt?.autoContinueOnCorrect !== undefined 
 ? !!activeCustomPrompt.autoContinueOnCorrect 
 : !hasSuccessText; 
 const successText = hasSuccessText ? successTextRaw : getShortSuccess(); 
 ...
 setCarryNote({
 type: "ok",
 text: typeof successTextRaw === "string" && successTextRaw.trim() ? successTextRaw.trim() : "Well done!",
 });
 askedRef.current = { 
 ...(askedRef.current || {}), 
 customAsked: { ...((askedRef.current && askedRef.current.customAsked) || {}), [promptId]: true }, 
 ...
 : Array.isArray(expectedChoice) 
 ? expectedChoice.length === 2 
 ? `${expectedChoice[0]} or ${expectedChoice[1]}` 
 : expectedChoice.join(", ") 
 : String(expectedChoice); 
 if (!correct) { 
 const hasRevealTextW = activeCustomPrompt?.revealText != null && activeCustomPrompt?.revealText !== ""; 
 const autoAdvanceWrong = 
 !activeCustomPrompt?.noContinue && 
 !endHandAfter && 
 (activeCustomPrompt?.autoContinueOnCorrect !== undefined 
 ? !!activeCustomPrompt.autoContinueOnCorrect 
 ...
 setFeedback(null); 
 setPlayDecisionReveal(null); 
 setCarryNote({ type: "error", text: `Not quite — the answer was ${correctAnswerText}.` }); 
 afterManualTrick(completedRoundIdx); 
 return; 
 } 
 ...
 <p className="ct-revealParagraph"> 
                    {REVEAL_GOOD_TRY}The correct answer is{" "}
                    <strong>
                      <TextWithColoredSuits text={correctAnswerText} />
                    </strong>
                    .
 </p> 
 ...
 const hasRevealText = activeCustomPrompt?.revealText != null && activeCustomPrompt?.revealText !== "";
 // Default: a correct yes/no answer with no teaching reveal flows straight to the next screen 
 // (with a carried "Well done"). A reveal/explanation keeps its own screen. 
 ...
 ? !!activeCustomPrompt.autoContinueOnCorrect 
 : !hasRevealText); 
 if (autoContinueDecision) {
 setPlayDecisionReveal(null);
 setCarryNote({
 type: "ok",
 text:
 typeof activeCustomPrompt?.successText === "string" && activeCustomPrompt.successText.trim()
 ? activeCustomPrompt.successText.trim()
 : "Well done!",
 });
 afterManualTrick(completedRoundIdx);
 ...
 setAskedTick((t) => t + 1); 
 const infoContinueToRoundIdx = Number(activeCustomPrompt?.infoContinueToRoundIdx); 
 if (Number.isFinite(infoContinueToRoundIdx) && lastRoundIdx >= 0) { 
 setPromptStep(null); 
 const through = Math.max(-1, Math.min(lastRoundIdx, infoContinueToRoundIdx)); 
 const start = Math.max(0, completedRoundIdx + 1); 
 if (start <= through) { 
 playSegment(start, through, () => { 
 setCompletedRoundIdx(through); 
 afterManualTrick(through); 
 }); 
 return; 
 } 
 afterManualTrick(Math.max(-1, completedRoundIdx)); 
 return; 
 } 
 setPromptStep(null); 
  // Question-only / no-scripted-play hands: optional skip DONE + Replay and move straight to next hand.
  const hasScriptedRoundPlays =
    Array.isArray(puzzle?.rounds) &&
    puzzle.rounds.some((round) => Array.isArray(round?.plays) && round.plays.length > 0);
  if (
    puzzle?.promptOptions?.infoContinueSkipsDoneScreen &&
    !hasScriptedRoundPlays &&
    !getNextCustomPrompt(completedRoundIdx)
  ) {
 setCompletedProblemIds((prev) => { 
 ...
 if (isDummyFollow && activeCustomPrompt.playCardRequireCorrect) { 
    ...
    const suitOk =
      expectedSuit == null ||
      expectedSuit === "" ||
      (Array.isArray(expectedSuit)
        ? expectedSuit.map((s) => String(s)).includes(String(card.suit))
        : card.suit === expectedSuit);
    const rankOk =
      expectedRank == null ||
      expectedRank === "" ||
      (Array.isArray(expectedRank)
        ? expectedRank.map((r) => String(r)).includes(String(card.rank))
        : String(card.rank) === String(expectedRank));
 if (!suitOk || !rankOk) { 
 ...
 setPlayedFromHand({ LHO: {}, DUMMY: {}, RHO: {}, DECLARER: {} }); 
 setRemainingHands(buildInitialRemainingHands(puzzle)); 
 // Clear any cards left on the table so the reveal shows the starting position only (no duplicates). 
 setTrickCards({ LHO: null, DUMMY: null, RHO: null, DECLARER: null }); 
 }, [promptStep, puzzle]); 
 ...
 const resolvedByPromptId = puzzle?.promptOptions?.auctionResolvedTextByPromptId; 
 const activeId = activeCustomPrompt?.id; 
 if (resolvedByPromptId && typeof resolvedByPromptId === "object" && activeId) { 
 const revealKey = `${activeId}--reveal`; 
 if ((promptStep === "PLAY_DECISION_REVEAL" || promptStep === "PLAY_CARD_REVEAL") && resolvedByPromptId[revealKey]) { 
 return resolvedByPromptId[revealKey]; 
 } 
 if (resolvedByPromptId[activeId]) return resolvedByPromptId[activeId]; 
 } 
 ...
 if (puzzle?.promptOptions?.auctionShowResolvedOnlyAfterPromptAsked) { 
 const asked = askedRef.current?.customAsked || {}; 
 return asked[infoId] ? resolved : base; 
 } 
 ...
 auctionText, 
 puzzle?.promptOptions?.auctionResolvedTextByPromptId, 
 puzzle?.promptOptions?.auctionShowResolvedDuringInfoPromptId, 
 puzzle?.promptOptions?.auctionResolvedText, 
 puzzle?.promptOptions?.auctionShowResolvedOnlyAfterPromptAsked, 
 promptStep, 
 askedTick, 
 activeCustomPrompt?.id, 
 ]); 
 ...
 const keepAuctionDuringReveal = 
 !!puzzle?.promptOptions?.showAuctionDuringPlayDecisionReveal && 
 (promptStep === "PLAY_DECISION_REVEAL" || promptStep === "PLAY_CARD_REVEAL"); 
 const hideAuctionNow = 
 (!keepAuctionDuringReveal && !!puzzle?.promptOptions?.hideAuction) || 
 (!keepAuctionDuringReveal && !!activeCustomPrompt?.hideAuction) || 
 (!keepAuctionDuringReveal && promptStep === "DONE" && !!puzzle?.promptOptions?.hideAuctionOnDone) || 
 (promptStep === "PLAY_DECISION_REVEAL" && !puzzle?.promptOptions?.showAuctionDuringPlayDecisionReveal); 
 ...
 {!!effectiveThemeLabel && ( 
 <div 
className={`ct-themeLabel ct-themeLabel--rail ${puzzle?.promptOptions?.promptThemeTint === "points" ? "ct-themeLabel--themePoints" : ""} ${puzzle?.promptOptions?.promptThemeTint === "active" ? "ct-themeLabel--themeActive" : ""} ${puzzle?.promptOptions?.promptThemeTint === "respond" ? "ct-themeLabel--themeRespond" : ""} ${puzzle?.promptOptions?.promptThemeTint === "1nt" ? "ct-themeLabel--theme1nt" : ""} ${puzzle?.promptOptions?.promptThemeTint === "matchpoints" ? "ct-themeLabel--themeMatchpoints" : ""} ${puzzle?.promptOptions?.promptThemeTint === "handEval" ? "ct-themeLabel--themeHandEval" : ""} ${puzzle?.promptOptions?.promptThemeTint === "doubles" ? "ct-themeLabel--themeDoubles" : ""} ${puzzle?.promptOptions?.promptThemeTint === "knockAce" ? "ct-themeLabel--themeKnockAce" : ""} ${isCyanDeclarerThemeTint(puzzle?.promptOptions?.promptThemeTint) ? "ct-themeLabel--themeDrawTrumps" : ""} ${puzzle?.promptOptions?.promptThemeTint === "ruffingLot" ? "ct-themeLabel--themeRuffingLot" : ""} ${puzzle?.promptOptions?.promptThemeTint === "enemyFive" ? "ct-themeLabel--themeEnemyFive" : ""} ${puzzle?.promptOptions?.promptThemeTint === "twoLevel" ? "ct-themeLabel--themeTwoLevel" : ""} ${puzzle?.promptOptions?.promptThemeTint === "preempt" ? "ct-themeLabel--themePreempt" : ""} ${puzzle?.promptOptions?.promptThemeTint === "respondToDouble" ? "ct-themeLabel--themeRespondToDouble" : ""} ${puzzle?.promptOptions?.promptThemeTint === "splinters" ? "ct-themeLabel--themeSplinters" : ""} ${puzzle?.promptOptions?.promptThemeTint === "reverses" ? "ct-themeLabel--themeReverses" : ""} ${puzzle?.promptOptions?.promptThemeTint === "showHand" ? "ct-themeLabel--themeShowHand" : ""} ${puzzle?.promptOptions?.promptThemeTint === "see43" ? "ct-themeLabel--themeSee43" : ""} ${puzzle?.promptOptions?.promptThemeTint === "slamJudgment" ? "ct-themeLabel--themeSlamJudgment" : ""} ${puzzle?.promptOptions?.promptThemeTint === "deadlyDuck" ? "ct-themeLabel--themeDeadlyDuck" : ""} ${puzzle?.promptOptions?.promptThemeTint === "openingLead" ? "ct-themeLabel--themeOpeningLead" : ""} ${puzzle?.promptOptions?.promptThemeTint === "loserCount" ? "ct-themeLabel--themeLoserCount" : ""} ${puzzle?.promptOptions?.promptThemeTint === "rebidFundamentals" ? "ct-themeLabel--themeRebidFundamentals" : ""} ${puzzle?.promptOptions?.promptThemeTint === "takeoutPenalty" ? "ct-themeLabel--themeTakeoutPenalty" : ""} ${puzzle?.promptOptions?.promptThemeTint === "defenceTrumpCount" ? "ct-themeLabel--themeDefenceTrumpCount" : ""}`} 
 > 
 {effectiveThemeLabel} 
 </div> 
 )} 
 ...
 puzzle?.promptOptions?.auctionHighlightTint === "alert" ? "ct-auctionCall--ringAlert" : "" 
 }`.trim()} 
 > 
 <span className="ct-auctionCallText">{c.text}</span> 
 {c.suitSym ? ( 
 ...
 <div 
 className={`ct-auctionHighlightNote ${ 
 puzzle?.promptOptions?.auctionHighlightTint === "alert" ? "ct-auctionHighlightNote--alert" : "" 
 } ${ 
 puzzle?.promptOptions?.auctionHighlightCall && puzzle?.promptOptions?.auctionHighlightNote 
 ? "ct-auctionHighlightNote--pairedCallRing" 
 : "" 
 }`.trim()} 
 > 
 {puzzle.promptOptions.auctionHighlightNote} 
...
/> 
 ...
 {carryNote && (
 <div className={`ct-carryNote ct-carryNote--${carryNote.type === "error" ? "error" : "ok"}`} role="status">
 <span className="ct-carryNote-tick" aria-hidden="true">{carryNote.type === "error" ? "✕" : "✓"}</span>
 <span>{carryNote.text}</span>
 </div>
 )}
...
<FormattedRevealText text={puzzle.promptOptions.persistentPlayCardHint} /> 
 ...
 key={c.key} 
 to={c.path} 
className={`ct-categoryTab ${c.key === activeCategoryTabKey ? "ct-categoryTab--active" : ""}`} 
 role="tab" 
aria-selected={c.key === activeCategoryTabKey} 
 > 
 {c.label} 
