...
class Coupons extends Component {
    ...
    tokenForApi = (token) => {
        ...
        if (normalized === "klinger" || normalized === "easts" || normalized === "milne" || normalized === "harbourview") return "blue";
        return normalized;
    };
    ...
    validateUserToken = () => {
        ...
        return $.ajax({
            ...
            data: `token=${encodeURIComponent(this.tokenForApi(this.state.token))}`,
            dataType: "json",
        }).then(
            (data) => {
                // console.log(status);
