...
export const noDuplicateCardsOfSameSuit = (North, East, South, West, suit) => {
    ...
    return concatenatedString.length == concatenatedString.split("").unique().length;
}


export const fetchDataChunk = (from, howMany, orderBy) => {
    return database.collection(from)
...
export const filterArticles = (articlesArray, {category, difficulty, searchString }) => {
    // console.log(articlesArray);
    // console.log(category, difficulty, searchString);
    // console.log(searchString == "");
    if (category == '""' && difficulty == '""' && (searchString == "" || searchString == '""')) {
        // console.log("IN HERE");
        // console.log(articlesArray);
    ...
    return articlesArray.filter(article => {
        ...
        if (searchString && searchString !== '""') {
            toSearch = [article.category, getDifficultyStr(article.difficulty), article.title, article.teaser ].join(" ").toLowerCase();
...
export const filterQuizzes = (articlesArray, {category, difficulty, searchString, hide, quizScores = {}}, quizScoresDB) => {
    ...
    return articlesArray.filter(article => {
        ...
        let toSearch;
        if (searchString !== '""') {
            toSearch = [article.quizType, getDifficultyStr(article.difficulty), article.title, article.teaser ].join(" ").toLowerCase();
            searchStringMatch = toSearch.includes(searchString.toLowerCase());
        }
        ...
        return (showCompletedDB && showCompleted) &&
               categoriesMatch &&
               difficultiesMatch &&
               searchStringMatch;
    });
...
export const parseDocumentIntoJSX = (documentString, quiz=false, callback=undefined, quizType=undefined) => {
    // console.log(documentString);
    // console.log(documentString.split(/(<MakeBoard .*" \/>)/));
    // let count = 0;
    // let re = /(<MakeBoard .* \/>)/;
    ...
    const articleDataArray = documentString.split(/(<MakeBoard\s.*?\s\/>)/)
        .map((substring, idx) => {
            if (!substring.includes("MakeBoard")) {
                ...
                return (
                    <div key={`${idx}x`} className="Display-board_container">
                        <br/>
                        <MakeBoard {...makeBoardObjectFromString(substring, true)}
                                   getBidding={callback}
                                   isQuiz={quiz}
                                   quizType={quizType}
                        />
