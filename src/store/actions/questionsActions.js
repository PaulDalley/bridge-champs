...
import database, {questionsRef, membersDataRef, countsRef, firebase, articlesRef, articleRef} from '../../firebase/config';
...
export const submitQuestion = data => {
    return dispatch => {
        ...
        const memRef = membersDataRef.doc(data.uid);
        ...
        batch.update(memRef, {
            blocked: true,
            lastQuestion: firebase.firestore.FieldValue.serverTimestamp(),
        });
        batch.commit()
            .then((r) => {
...
export const getArticles = () => {
    return dispatch => {
        articlesRef.orderBy('createdAt', 'desc')
            .get()
            .then(snapshot => {
                const articles = [];
                snapshot.forEach(childSnapshot => {
...
export const getTournamentArticles = () => {
    return dispatch => {
        articlesRef.where("category", "==", "Tournament")
            .get()
            .then(snapshot => {
